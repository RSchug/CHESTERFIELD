// Expression Name: Professional
//      Target Portlet: License Professional
//      Execute Order:
...
/------------------------------------------------------------*/
var thisForm = expression.getValue("LP::FORM"); // be sure to update this as neccessary depending on target portlet.
var fieldsASI = [];
fieldsASI["licensenbr"] = expression.getValue("LP::professionalModel*licensenbr");
fieldsASI["licensetype"] = expression.getValue("LP::professionalModel*licensetype");
fieldsASI["typeFlag"] = expression.getValue("LP::professionalModel*typeFlag");
fieldsASI["salutation"] = expression.getValue("LP::professionalModel*salutation");
...
capFields["fileDate"] = expression.getValue("CAP::capModel*fileDate");
...
try {
    ...
    var capIdString = capId1.value + "-" + capId2.value + "-" + capId3.value;
    var capId = aa.cap.getCapID(capId1.value, capId2.value, capId3.value).getOutput();
    if (!capId && capFields["altID"]) {
        logDebug("Using capId: " + capFields["altID"].value)
        var capId = aa.cap.getCapID(capFields["altID"].value).getOutput();
    }
    if (capId) {
    ...
    // Custom Code here.
    if (fieldsASI["licensenbr"].value != null && !fieldsASI["licensenbr"].value.equals("") && fieldsASI["licensetype"].value != null && !fieldsASI["licensetype"].value.equals("")) {
        var estValue = null;
        if (capId) {
        ...
        var jobCostMax = null;
        var jobCostRank = fieldsASI["salutation"].getValue();
        if (typeof (jobCostMaxValueMap[jobCostRank]) != "undefined") jobCostMax = jobCostMaxValueMap[jobCostRank];
        logDebug("TEST: "
            + ", Lic #: " + fieldsASI["licensenbr"].getValue()
            + ", Type: " + fieldsASI["licensetype"].getValue()
            + ", Flag: " + fieldsASI["typeFlag"].getValue()
            + ", Rank: " + fieldsASI["salutation"].getValue()
            + ", jobCostMax: " + jobCostMax
            + ", estValue: " + estValue);
        if (estValue != null && jobCostMax != null && estValue > jobCostMax) {
            message = "Estimated Cost of Construction (" + estValue + ") is over their Rank limit: " + jobCostMax
            logDebug(message);
            cancel = true;
        }
    }
